...
function kochline(size: number, xstart: number, ystart: number, angle: number, color = randomColor()) {
  ...
  if (size > 10) {
    ...
    ctx.stroke();
    setTimeout(function () {
      kochline(size / 3, xstart, ystart, angle, color);
      kochline(size / 3, x2, y2, angle, color);
      kochline(size / 3, x1, y1, angle + 5 * Math.PI / 3, color);
      kochline(size / 3, x1 + Math.sin(angle - Math.PI / 3) * size / 3, y1 - Math.cos(angle - Math.PI / 3) * size / 3, angle + Math.PI / 3, color);
    }, 500)
  }
}

kochline(canvas.width, 0, canvas.height / 2, Math.PI / 2, 'black');