...
$("#greeter").on("click", function () {
  $.get(window.baseUrl + "/greeter", {}).done(function (data) {
    if ("error" in data && data["error"] == "Please provide a name and a title!") {
      handle("#greeter_response", "OK - without name and title");
    } else {
      handle("#greeter_response", "not ok - without name and title", "Greeter", data);
    }
  }).error(function (err) {
